...
<head>
    ...
    <style>
        ...
        .btn-infox{
            border: 0;
            background: linear-gradient(90deg, var(--info1), var(--info2));
        ...
        .btn-infox:hover{ transform: translateY(-1px); filter: brightness(1.04); }
        ...
        .qty-btn:hover{ background: rgba(14,165,233,.12); }
        .qty-input{
            border: 0 !important;
...
<div class="container py-4">
    ...
    <div class="topbar d-flex flex-wrap justify-content-between align-items-center gap-2 mb-3">
        <div>
            <div class="d-flex align-items-center gap-2">
                <h4 class="mb-0 fw-bold">🔁 Transfer Stock</h4>
                <span class="badge-soft">Move stock</span>
            </div>
            <div class="muted small">Use this when moving items to another branch / kitchen / store.</div>
        </div>
    ...
    <div class="card card-soft">
        <div class="card-head">
            <div class="fw-bold">New Transfer</div>
            <div class="muted small">Choose product → qty → destination note.</div>
        </div>

        <div class="card-body p-4">
            <div class="info-panel mb-3">
                <div class="fw-bold">ℹ️ Note</div>
                <div class="muted small">Qty cannot be greater than current stock. (Validate backend too.)</div>
            </div>

            <form id="transferForm" method="POST" action="{{ route('admin.transactions.transfer.store') }}" class="row g-3">
                @csrf
                ...
                {{-- Destination Note --}}
                <div class="col-md-8">
                    <label class="form-label fw-semibold">Destination / Note</label>
                    <div class="icon-input">
                        <span class="icon">📍</span>
                        <input id="noteInput" type="text" name="note" class="form-control"
                               placeholder="Transfer to branch A / kitchen / warehouse">
                    </div>
                </div>

                {{-- Actions --}}
                <div class="col-12 d-flex gap-2 flex-wrap mt-2">
                    <button id="saveBtn" class="btn btn-infox px-4" type="submit">
                        <span class="btn-text">🔁 Save Transfer</span>
